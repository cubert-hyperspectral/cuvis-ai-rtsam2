...
class SAM2CameraPredictor(SAM2Base):
    ...
    def _manage_memory_obj2(self, frame_idx, current_out):
        output_dict = self.condition_state["output_dict"]
        cond_frame_outputs = output_dict["cond_frame_outputs"]
        self.condition_state["output_dict"]["cond_frame_outputs"][self.frame_idx] = current_out

        key_list = [key for key in output_dict["cond_frame_outputs"]]
        #! TODO: better way to manage memory
        if len(cond_frame_outputs) > 2*self.num_maskmem:
            for t in range(0, len(cond_frame_outputs) - 2*self.num_maskmem):
                # key, Value = non_cond_frame_outputs.popitem(last=False)
                _ = cond_frame_outputs.pop(key_list[t], None)
        # print("1: ", self.condition_state["output_dict"]["cond_frame_outputs"].get(self.frame_idx, None))
        output_dict_per_obj = self.condition_state["output_dict_per_obj"]
        # print(output_dict["cond_frame_outputs"][self.frame_idx]["maskmem_features"])
    ...
    def _add_temp_output_per_object(self, frame_idx, current_out, storage_key):
        """
        Split a multi-object output into per-object output slices and add them into
        `output_dict_per_obj`. The resulting slices share the same tensor storage.
        """
